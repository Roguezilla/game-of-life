...
def alive(x, y):
	...
	for xx, yy in neighbours:
		try:
			if cells[x+xx][y+yy] == 'o' and x+xx in range(len(cells)) and y+yy in range(len(cells[0])):
				num_of_neighbours += 1
...
def update(dt):
	...
	for x in range(len(dc_cells)):
		for y in range(len(dc_cells[0])):
			live = alive(x,y)
			if dc_cells[x][y] == 'o' and live < 2:
				dc_cells[x][y] = '.'
			elif dc_cells[x][y] == 'o' and (live == 2 or live == 3):
				dc_cells[x][y] = 'o'
			elif dc_cells[x][y] == 'o' and live > 3:
				dc_cells[x][y] = '.'
